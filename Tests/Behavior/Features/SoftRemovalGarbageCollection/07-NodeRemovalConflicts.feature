Feature: Tests for soft removal garbage collection with impending conflicts caused by hard removals
  ...
  Scenario: Garbage collection will ignore a soft removal if one of the node's descendants has an unpublished hard removal in another workspace
    When the command TagSubtree is executed with payload:
      | Key                          | Value                 |
      | workspaceName                | "live"                |
      | nodeAggregateId              | "nodingers-cat"       |
  ...
  Scenario: Garbage collection will transform a soft removal if there only is a removal on an unrelated node aggregate
    When the following CreateNodeAggregateWithNode commands are executed:
      | nodeAggregateId     | parentNodeAggregateId  | nodeTypeName       |
      | nodingers-other-cat | sir-david-nodenborough | Neos.Neos:Document |
    And the command RebaseWorkspace is executed with payload:
      | Key           | Value            |
      | workspaceName | "user-workspace" |
    ...
    And the command RemoveNodeAggregate is executed with payload:
      | Key                          | Value                 |
      | nodeAggregateId              | "nodingers-other-cat" |
      | coveredDimensionSpacePoint   | {"example":"source"}  |
      | nodeVariantSelectionStrategy | "allSpecializations"  |
    And the command RebaseWorkspace is executed with payload:
      | Key           | Value            |
  ...
  Scenario: Garbage collection will transform a soft removal if there only is a removal in an unrelated dimension space point
