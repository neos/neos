Feature: Tests for soft removal garbage collection objections
  ...
  # CreateNodeVariant conflict prevention (for descendants)
  Scenario: Garbage collection will ignore a soft removal if the node has unpublished newly created variants in another workspace
    When the command TagSubtree is executed with payload:
      | Key                          | Value                 |
      | workspaceName                | "live"                |
  ...
  Scenario: Garbage collection will ignore a soft removal if the node has unpublished reference changes in another workspace
    When the command TagSubtree is executed with payload:
      | Key                          | Value                 |
      | workspaceName                | "live"                |
      | nodeAggregateId              | "nodingers-cat"       |
